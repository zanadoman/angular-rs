...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
  dotenvy::dotenv()?;
  let (writer, _guard) = tracing_appender::non_blocking(rolling::daily("logs", "log"));
...
async fn serve() -> Result<(), Box<dyn Error + Send + Sync>> {
  let listener = TcpListener::bind(SocketAddr::from((
    ...
    env::var("APP_PORT").map_or(Ok(8080), |p| p.parse())?,
  )))
  .await?;
  ...
  axum::serve(
    ...
    angular_rs::new(
      ...
      AsyncSmtpTransport::<Tokio1Executor>::builder_dangerous(
        env::var("SMTP_ADDRESS").unwrap_or_default(),
      )
      .port(env::var("SMTP_PORT").map_or(Ok(1025), |p| p.parse())?)
      .tls(Tls::Wrapper(
        TlsParameters::builder(env::var("SMTP_ADDRESS").unwrap_or_default())
          .dangerous_accept_invalid_hostnames(cfg!(debug_assertions))
          .dangerous_accept_invalid_certs(cfg!(debug_assertions))
          .build()?,
      ))
      ...
      .build(),
      env::var("APP_SESSION_EXPIRY").map_or(Ok(0), |e| e.parse())?,
      Path::new("dist/angular-rs/browser"),
    )
    .await?,
  )
