...
pub struct User {
  ...
  pub id: i64,
  pub username: String,
  #[serde(serialize_with = "serialization::default_serializer")]
  #[schema(write_only)]
  pub password: String,
...
LIMIT 1
  ...
  pub fn create<'a, 'c, A>(
    &'a mut self,
    conn: A,
  ) -> impl Future<Output = Result<(), AppError>> + Send + 'a
  where
    A: Acquire<'c, Database = Postgres> + Send + 'a,
  {
    async move {
      ...
      if self.username.len() < 3 {
        errs.push(validation::invalid!(self, username, Validation::TooShort));
      } else if 50 < self.username.len() {
        errs.push(validation::invalid!(self, username, Validation::TooLong));
      } else if Self::where_username(&mut *conn, &self.username, true)
        .await?
        .is_some()
      {
        errs.push(validation::invalid!(self, username, Validation::Duplicate));
      }
      if self.password.len() < 8 {
        errs.push(validation::invalid!(self, password, Validation::TooShort));
      }
