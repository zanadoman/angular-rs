...
export class RootPage {
  ...
  protected async onRegister(): Promise<void> {
    try {
      RootPage.showObject(
        await firstValueFrom(this._httpClient.post(`${API}/register`, this.registerForm.value)),
      );
    } catch (err) {
  ...
  protected async onLogin(): Promise<void> {
    try {
      RootPage.showObject(
        await firstValueFrom(this._httpClient.post(`${API}/login`, this.loginForm.value)),
      );
    } catch (err) {
      RootPage.showError(err);
    }
  ...
  protected async onLogout(): Promise<void> {
    try {
      RootPage.showResponse(
        await firstValueFrom(
          this._httpClient.post(`${API}/logout`, null, {
            observe: 'response',
          }),
        ),
      );
